...
def clear_memory():
    ...
    # Clear any lingering torch tensors
    gc.collect()
...
def train_mlp_with_params(X_train, y_train, X_val, y_val, params):
    ...
    Includes memory cleanup to prevent crashes during Optuna trials.
    """
    try:
        model = SimpleMLP(
            X_train.shape[1],
        ...
        del model, criterion, optimizer, X_t, y_t
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        ...
        # If MLP training fails, return a low score to let Optuna continue
        gc.collect()
        return 0.33  # Random chance for 3-class
...
def run_loso_with_fixed_params(model_name, norm_name, X, y, subjects, best_params):
    ...
    This is the HYBRID approach: Optuna optimizes once during CV, then LOSO
    uses those fixed params for all 53 folds. This is 44x faster than nested
    Optuna while maintaining scientific validity.
    """
...
def main():
    print('=' * 70)
    print('PHASE 4 - FULL MODEL TRAINING + LOSO VALIDATION (WITH OPTUNA)')
    print('=' * 70)
    ...
    # Define valid experiments (6 total - SimpleMLP EXCLUDED due to segfault crashes)
    # SimpleMLP causes C-level crashes that Python cannot catch
    experiments = [
        ('LightGBM', 'per_subject_baseline', per_subject_baseline_norm),
        ('LightGBM', 'global_zscore', global_zscore_norm),
        ('LightGBM', 'raw', raw_norm),
        # SimpleMLP EXCLUDED - causes segmentation faults
    ...
    for i, (model_name, norm_name, norm_fn) in enumerate(experiments):
        experiment_key = f'{model_name}_{norm_name}'

        # Skip if already completed
        if experiment_key in completed_keys:
            print(f'\n[{i+1}/8] {model_name} + {norm_name} - SKIPPED (already completed)')
            continue
        ...
        try:
            ...
            # Clear memory between CV and LOSO
            clear_memory()
            ...
            # Add results and mark as completed
            cv_results.append(cv_result)
            loso_results.append(loso_result)
            completed_keys.append(experiment_key)

            # SAVE CHECKPOINT IMMEDIATELY after each experiment
            save_checkpoint(RESULTS_DIR, completed_keys, cv_results, loso_results)
            save_incremental_results(RESULTS_DIR, cv_results, loso_results)

            # Clear memory after experiment completes
            clear_memory()
            ...
            # Clear memory and continue to next experiment
            clear_memory()
            continue

    # Report on failed experiments
    if failed_experiments:
        print(f'\n[WARNING] {len(failed_experiments)} experiment(s) failed:')
        for fe in failed_experiments:
            print(f'  - {fe["experiment"]}: {fe["error"]}')

    # Check if we have any results
    if not cv_results or not loso_results:
    ...
    print('=' * 70)
    print(f'Experiments completed: {len(completed_keys)}/8')
    if failed_experiments:
    ...
    partial_cv = RESULTS_DIR / 'cv_leaderboard_partial.csv'
    partial_loso = RESULTS_DIR / 'loso_leaderboard_partial.csv'
    if partial_cv.exists():
        partial_cv.unlink()
    if partial_loso.exists():
        partial_loso.unlink()
