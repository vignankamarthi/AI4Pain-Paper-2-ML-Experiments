"""
Phase 6 Step 5: Hierarchical Binary Classification (BASELINE-ONLY)

MIRRORS Phase 5 but with baseline-only labeling:
...
Uses top 3 configurations from Step 4 leaderboard.

HYBRID OPTUNA APPROACH (44x faster):
- Optuna optimizes hyperparameters ONCE using 5-fold CV on full dataset
- LOSO validation uses those fixed optimal params for all 53 folds
- This avoids nested optimization (50 trials x 53 folds = 2650 per stage)
- Trade-off: Sacrifices per-fold optimization for practical runtime
"""
...
# Class mappings - BASELINE ONLY (rest segments EXCLUDED)
CLASS_MAP_3CLASS = {'baseline': 0, 'low': 1, 'high': 2}
CLASS_MAP_STAGE1 = {'baseline': 0, 'low': 1, 'high': 1}  # No Pain vs Pain
...
PHASE4_RESULTS = PROJECT_ROOT / 'results' / 'phase6_step4_full_training'
RESULTS_DIR = PROJECT_ROOT / 'results' / 'phase6_step5_hierarchical'
...
def load_checkpoint(results_dir):
    ...
    if checkpoint_file.exists():
        ...
        print(f'  Loaded checkpoint: {len(checkpoint["completed"])} configs completed')
        return checkpoint
...
def save_checkpoint(results_dir, completed, all_results):
    ...
    print(f'    [CHECKPOINT SAVED] {len(completed)} configs completed')
...
def optimize_only(X_train, y_train, model_type='LightGBM'):
    ...
    HYBRID APPROACH: This is called once before LOSO to find optimal hyperparameters.
    Those params are then used across all 53 LOSO folds.
    """
...
def train_with_fixed_params(X_train, y_train, params, model_type='LightGBM'):
    ...
    HYBRID APPROACH: This is called inside each LOSO fold with pre-optimized params.
    """
    if model_type == 'LightGBM':
...
def run_hierarchical_loso(df, config):
    ...
    print(f"\n  Running LOSO for {config['model']} + {config['normalization']}...")

    # === STEP 1: OPTIMIZE PARAMS ONCE USING FULL DATA ===
    # Apply normalization to full dataset for optimization
    if config['normalization'] == 'global_zscore':
        scaler_full = StandardScaler()
        X_all_norm = scaler_full.fit_transform(X_all)
    ...
    # Stage 1 optimization (No Pain vs Pain)
    print("    Stage 1 (Pain Detection) Optuna optimization...")
    params_s1, cv_s1 = optimize_only(X_all_norm, y_stage1, config['model'])
    print(f"      Best CV accuracy: {cv_s1:.4f}")

    # Stage 2 optimization (Low vs High on pain samples only)
    pain_mask_full = y_3class > 0
    X_pain_full = X_all_norm[pain_mask_full]
    y_stage2_full = (y_3class[pain_mask_full] - 1).astype(np.int64)

    print("    Stage 2 (Intensity) Optuna optimization...")
    if len(np.unique(y_stage2_full)) > 1:
        params_s2, cv_s2 = optimize_only(X_pain_full, y_stage2_full, config['model'])
        print(f"      Best CV accuracy: {cv_s2:.4f}")
    else:
        params_s2 = None
        print("      [WARN] Insufficient class diversity for Stage 2 optimization")

    # === STEP 2: RUN LOSO WITH FIXED PARAMS ===
    print("    Running LOSO with fixed params (53 folds)...")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(logo.split(X_all, y_3class, subjects)):
        ...
        if config['normalization'] == 'global_zscore':
            ...
            # For test subject, we can't use their baseline (LOSO)
            # Use global normalization for test
        ...
        # === STAGE 1: No Pain vs Pain (fixed params) ===
        ...
        # === STAGE 2: Low vs High (fixed params) ===
        ...
        if params_s2 is not None and len(np.unique(y_train_stage2)) > 1:
        ...
        for i in range(len(pred_stage1)):
            if pred_stage1[i] == 0:  # Predicted No Pain
                final_pred[i] = 0
            else:  # Predicted Pain -> use Stage 2
                if model_s2 is not None:
                    x_test_row = X_test_norm[i:i+1]
                    s2_pred = model_s2.predict(x_test_row)[0]
                    final_pred[i] = s2_pred + 1
                else:
                    final_pred[i] = 1  # Default to low pain

        # Calculate 3-class accuracy
        acc_3class = balanced_accuracy_score(y_test_3class, final_pred)

        # Stage 2 accuracy (only on actual pain samples)
...
def main():
    ...
    completed_configs = set(checkpoint['completed'])
    ...
    for config in configs:
        ...
        try:
            ...
            save_checkpoint(RESULTS_DIR, list(completed_configs), all_results)

        except Exception as e:
            print(f'    [ERROR] {config_name} failed: {str(e)}')
            failed_configs.append({'config': config_name, 'error': str(e), 'timestamp': datetime.now().isoformat()})
            with open(RESULTS_DIR / 'failed_configs.json', 'w') as f:
                json.dump(failed_configs, f, indent=2)

        # Clear memory after config
        clear_memory()
